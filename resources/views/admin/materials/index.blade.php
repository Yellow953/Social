...
@section('content')
    <div class="container-fluid">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h2 class="fw-bold mb-1" style="color: #c2410c;"><i class="fas fa-play-circle me-2"
                ...
                <form method="GET" action="{{ route('admin.materials.index') }}" class="row g-2 align-items-end">
                    ...
                    <div class="col-md-2">
                        <select name="type" class="form-select">
                            ...
                            <option value="td" {{ ($filters['type'] ?? '') == 'td' ? 'selected' : '' }}>TD</option>
                            <option value="tp" {{ ($filters['type'] ?? '') == 'tp' ? 'selected' : '' }}>TP</option>
                            <option value="tc" {{ ($filters['type'] ?? '') == 'tc' ? 'selected' : '' }}>TC</option>
        ...
        <div class="card border-0 shadow-lg overflow-hidden"
            style="background: linear-gradient(135deg, #ffffff 0%, #f8fafc 100%); border-left: 4px solid #ec682a !important;">
                                ...
                                <tr>
                                    <td>
                                        <div>
                                            ...
                                            @if ($material->description)
                                                <br><small
                                                    class="text-muted">{{ Str::limit($material->description, 50) }}</small>
                                            @endif
                                        </div>
                                    </td>
                                    <td>{{ $material->course->name }}</td>
                                    <td><span
                                            class="badge bg-secondary">{{ ucfirst(str_replace('_', ' ', $material->type)) }}</span>
                                    </td>
                                    <td>
                                        <span
                                            class="material-lock-badge badge {{ $material->is_locked ? 'bg-warning' : 'bg-success' }}"
                                        ...
                                        <button type="button"
                                            ...
                                            data-material-id="{{ $material->id }}" style="border-radius: 8px;">
                                    ...
                                    <td class="text-end">
                                        <div class="d-flex gap-2 justify-content-end">
                                            <form method="POST"
                                                action="{{ route('admin.materials.duplicate', $material) }}"
                                                class="d-inline form-duplicate"
                                                data-confirm="Duplicate this material and all its files?">
                                                @csrf
                                                <button type="submit" class="btn btn-sm btn-info shadow-sm"
                                                    title="Duplicate" style="border-radius: 8px;">
                                            ...
                                            <form method="POST"
                                                ...
                                                <button type="submit" class="btn btn-sm btn-danger shadow-sm"
                                                    title="Delete" style="border-radius: 8px;">
                        ...
                        fetch('{{ url('admin/materials') }}/' + id + '/toggle-lock', {
                            ...
                            .then(function(data) {
                                if (data.is_locked) {
                                    ...
                                    btn.className =
                                        'btn btn-sm btn-outline-success ms-1 material-lock-toggle';
                                    btn.title = 'Unlock';
                                    ...
                                    btn.className =
                                        'btn btn-sm btn-outline-warning ms-1 material-lock-toggle';
                                    btn.title = 'Lock';
